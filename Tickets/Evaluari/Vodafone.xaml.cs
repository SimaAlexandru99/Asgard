...
namespace Asgard.Tickets.Evaluari
{
    ...
    public partial class Vodafone : Page
    {
        ...
        {
            if (emailAngajat.Text != string.Empty && emailSuperior.Text != string.Empty && numarClient.Text != string.Empty && ComboProiect.Text != string.Empty && dataApel.Text != string.Empty && tipApel.Text != string.Empty && notaIntrebare1.Text != string.Empty && notaIntrebare2.Text != string.Empty && notaIntrebare3.Text != string.Empty && notaIntrebare4.Text != string.Empty && notaIntrebare5.Text != string.Empty && notaIntrebare6.Text != string.Empty && notaIntrebare7.Text != string.Empty && notaIntrebare8.Text != string.Empty && notaIntrebare9.Text != string.Empty && notaIntrebare10.Text != string.Empty )
            {
                string connectionString = "server=192.168.100.18;port=3306;user=eoverart;password=P3CZV4pgc7jtT4z;database=asgard";
                string insertQuery = "INSERT INTO evaluari_vodafone VALUES (NULL, @email_agent, @email_evaluator, @numar_client, @proiect, @data_apel, @tip_apel, @deschidere_apel_nota, @deschidere_apel_observatii, @gdpr_nota, @gdpr_observatii, @identificare_nevoi_nota, @identificare_nevoi_observatii, @propunere_solutie_nota, @propunere_solutie_observatii, @tratare_obiectii_nota, @tratare_obiectii_observatii, @finalizare_apel_nota, @finalizare_apel_observatii, @atitudine_nota, @atitudine_observatii, @aplicatii_nota, @aplicatii_observatii, @rezolutie_apel_nota, @rezolutie_apel_observatii, @retentie_achiztie_nota, @retentie_achiztie_observatii, @observatii, NULL)";
                ...
                try
                {
                    using (MySqlConnection connection = new MySqlConnection(connectionString))
                    {
                        ...
                        using (MySqlCommand insertCommand = new MySqlCommand(insertQuery, connection))
                        {
                            ...
                            insertCommand.Parameters.AddWithValue("@identificare_nevoi_nota", notaIntrebare3.Text);
                            insertCommand.Parameters.AddWithValue("@identificare_nevoi_observatii", observatiiIntrebare3.Text);
                            insertCommand.Parameters.AddWithValue("@propunere_solutie_nota", notaIntrebare4.Text);
                            insertCommand.Parameters.AddWithValue("@propunere_solutie_observatii", observatiiIntrebare4.Text);
                            ...
                            insertCommand.Parameters.AddWithValue("@retentie_achiztie_nota", notaIntrebare10.Text);
                            insertCommand.Parameters.AddWithValue("@retentie_achiztie_observatii", observatiiIntrebare10.Text);
